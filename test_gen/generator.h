...
class TestGenerator
{
  ...
  private:
    ...
    static constexpr int rm_count = 18;
    ...
    std::uniform_int_distribution<> rm_inst_distr_{ 0, rm_count - 1 };
...
#define INST_CREATOR(name) Ins::MakeIns_##name,
    constexpr static std::array<Ins (*)(uint32_t, uint32_t, uint32_t),
                                rm_count>
        creator_table_rm_ = { INSTRUCTION_LIST_RM(INST_CREATOR) };

    constexpr static std::array<Ins (*)(int32_t, uint32_t, uint32_t), i_count>
        creator_table_i_ = { INSTRUCTION_LIST_I(INST_CREATOR) };

    constexpr static std::array<Ins (*)(int32_t, uint32_t, uint32_t), ls_count>
        creator_table_ls_ = { INSTRUCTION_LIST_LS(INST_CREATOR) };
#undef INST_CREATOR
};
