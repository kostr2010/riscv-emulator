...
void ReadElf::InitMemory() {
    memory_ = reinterpret_cast<uint8_t*>(mmap(NULL,             // Need our own realization of mmap
    ...
    if (munmap(memory_ + PageSize, Size - PageSize)) {          // Need our own realization of munmap
        throw ElfLoadException("Failed to init memory");        // Checked that enough memory is available
...
uint8_t *ReadElf::AllocateMemory(uint32_t vaddr, size_t length, int prot) {
    auto aligned_vaddr = AlignDown(vaddr);                                  // addr in mmap % 4096 == 0 else fault
    length = AlignUp(length + vaddr - aligned_vaddr);                       //  len in mmap % 4096 == 0 else fault
    ...
    void *paddr = mmap(Translate(aligned_vaddr), length, prot | PROT_WRITE, // Need our own realization of mmap
...
void *ReadElf::Memset(uint32_t vaddr, int fill_byte, size_t n_bytes) {
    return memset(Translate(vaddr), fill_byte, n_bytes);                    // Need our own realization of memset
}

void ReadElf::SetFD(int fd) {
    fd_ = fd;
}
